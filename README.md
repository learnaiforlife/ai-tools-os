...
Beta outputs are `release-local/AI-Tools-OS-<version>-arm64.{dmg,zip}` and `release-local/AI-Tools-OS-<version>-x64.{dmg,zip}`. `dist:mac:beta` creates complete ad-hoc signatures and runs package validation without Apple Developer credentials. To publish a beta, upload the tested bytes, verify their public checksums and update the site's beta manifest. Older unverified outputs must not be distributed as the current version.

The separate `dist:mac` stable release command requires a clean reviewed commit, Developer ID Application identity and notarization credentials. It verifies Gatekeeper/stapling and generates final checksums. These requirements do not disable the beta distribution path.

Package checks extract both ZIPs and mount both DMGs read-only, compare renderer/native files against source and verify signatures, the Applications shortcut and final metadata. Packaged UAT requires a styled visible interface and exercises editing, conflicts, profiles, restore, settings, statusline, special-file handling and small-window layouts. A separate reinstall check verifies retained preferences, prompts, parked resources and history; it is not proof of every older-version upgrade. CI runs on native Apple Silicon and Intel runners. Publication is manual; the build does not upload installers.
