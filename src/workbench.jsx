...
import { ScanIssues } from './scan-issues.jsx';
...
const PAGES = [['dashboard', 'Overview', 'grid'], ['sessions', 'Sessions', 'history'], ['tokens', 'Context explorer', 'tokens'], ['cleanup', 'Cleanup', 'tools'], ['skills', 'Skills', 'bolt'], ['memory', 'Memory & rules', 'memory'], ['mcp', 'MCP servers', 'mcp'], ['plugins', 'Plugins', 'grid'], ['config', 'Config files', 'file'], ['commands', 'Commands', 'terminal'], ['agents', 'Subagents', 'users'], ['prompts', 'Prompt library', 'bookmark'], ['skill-lab', 'Skill Lab', 'bolt'], ['memory-review', 'Memory Review', 'memory'], ['convert', 'Convert to Markdown', 'file'], ['security', 'Security review', 'shield'], ['tools', 'External tools', 'tools'], ['scan-issues', 'Scan report', 'file'], ['history', 'History & recovery', 'history'], ['tutorial', 'Getting started', 'book'], ['settings', 'Settings', 'settings']];
    ...
    {file && <>
      {resource.metadataWarning && <Notice>{resource.error} The complete resource is available below. Repair its metadata before transferring it; its provider may also reject this header.
        {!file.readonly && /\.(md|mdc)$/.test(resource.path) && <Button disabled={saving || dirty} onClick={async () => {
          setSaving(true); setError('');
          try { const result = await request('resource.repair.preview', { id: resource.id, parked: !!resource.parked, revision: file.revision }); setDraft(result.content); setDiff(true); setReading(false); setRepairReview(true); setRepairReviewed(false); }
          catch (e) { setError(e.message); } finally { setSaving(false); }
...
export function Workbench() {
  ...
  const run = useCallback(async (op = 'inventory', args = {}) => {
    ...
    try { const result = await request(op, args); if (result.resources) setData(result); setStatus(op === 'inventory' ? `Inventory refreshed from disk${result.issues?.length ? ' · Scan report has notices' : ''}` : 'Changes saved to disk'); return result; }
  ...
  const scanIssues = data.issues.filter(issue => issue.code !== 'LEGACY_DATA');
  ...
  if (KINDS.includes(page)) screen = <>{!['config', 'plugins'].includes(page) && <div className="wb-actions"><Button primary disabled={busy} onClick={() => setModal({ type: 'create', kind: page })}>New {page === 'mcp' ? 'MCP server' : page}</Button>{page === 'memory' && <span>Instruction files are listed by source; actual precedence is decided by the provider.</span>}</div>}
    ...
    {!!scanIssues.length && <section className="wb-card scan-summary"><h2>{data.scan?.limited ? 'Scan finished with coverage limits' : 'Scan completed with notices'}</h2><p>Healthy resources are ready. {data.scan?.skipped || 0} skipped-path notices · {data.scan?.warnings || 0} file warnings · {data.scan?.limited || 0} coverage limits.</p><Button onClick={() => navigate('scan-issues')}>View scan report</Button></section>}
  ...
  else if (page === 'scan-issues') screen = <ScanIssues data={data} edit={edit} busy={busy} retry={() => void run().catch(() => {})} settings={() => navigate('settings')} search={search} provider={provider} scope={scope} project={project} showDetails={experience.showDetails} />;
  ...
  return <div className="wb-app"><aside className="wb-sidebar"><div className="wb-brand"><span className="brand-mark" /> <div>AI Tools OS<small>Local configuration manager</small></div></div><nav aria-label="Main navigation">{PAGES.map(([key, title, icon]) => <React.Fragment key={key}>{({ dashboard: 'WORKSPACE', skills: 'RESOURCES', 'skill-lab': 'IMPROVE', 'scan-issues': 'MANAGE' })[key] && <div className="nav-section">{({ dashboard: 'WORKSPACE', skills: 'RESOURCES', 'skill-lab': 'IMPROVE', 'scan-issues': 'MANAGE' })[key]}</div>}<button key={key} aria-current={page === key ? 'page' : undefined} onClick={() => navigate(key)}><Icon name={icon} size={17} /><span>{title}</span>{key === 'scan-issues' && scanIssues.length > 0 && <small>{scanIssues.length}</small>}{KINDS.includes(key) && <small>{resources.filter(r => r.kind === key).length}</small>}</button></React.Fragment>)}</nav><p className="wb-sidebar-note">Your files. Your machine.<br />No cloud account required.</p></aside>
    <header className="wb-topbar">
      ...
      {error && <Notice error>{error}<Button onClick={() => setError('')}>Dismiss message</Button></Notice>}{screen}</main>
...
function Tools({ report }) {
  ...
  return <><Notice>Executable locations detected on this machine. Use Skill Lab for Claude Code evaluations or Convert to Markdown for managed converter installation and document extraction. Detection alone does not establish authentication or runtime health.</Notice>{tools ? tools.map(tool => <section className="wb-card" key={tool.name}><h2>{tool.name}</h2><p>{tool.path ? 'Executable found' : 'Not found in searched locations'}</p>{tool.path && <code>{tool.path}</code>}</section>) : <p>Checking executable locations…</p>}</>;
}
