import { AIControls, defaultAISettings, settingsFromPreferences } from './ai.jsx';
...
const defaultSettings = defaultAISettings;
...
function Settings({ value, change }) {
  return <><AIControls value={value} change={change} /><Field label="Repeats per test"><input aria-label="Run repeats" type="number" min="1" max="5" value={value.repeats} onChange={e => change({ ...value, repeats: Number(e.target.value) })} /></Field></>;
}
...
function Changes({ source, content, run, close }) {
  ...
  return <Dialog title="Review proposed changes" wide close={() => { if (!saving) close(); }}>
    ...
    <Button primary disabled={!reviewed || saving || source.readonly || source.content === content} onClick={async () => { setSaving(true); try { if (source.kind === 'prompts') await run('prompts.save', { prompt: { ...source, id: source.promptId, content } }); else await run('resource.write', { id: source.id, revision: source.revision, parked: false, content }); close(); } catch (e) { setError(e.message); } finally { setSaving(false); } }}>Apply reviewed changes</Button>
  </Dialog>;
}

function RunResult({ job, act, run, onCandidate, started }) {
  const [change, setChange] = useState(false), [feedback, setFeedback] = useState(job.feedback || ''), [cleanupIndex, setCleanupIndex] = useState(null), [cleanupSettings, setCleanupSettings] = useState(defaultSettings);
  const result = job.result, candidate = job.quick ? undefined : result?.candidate ?? job.candidate;
  useEffect(() => { let alive = true; request('preferences.ai.get').then(r => { if (alive) setCleanupSettings(current => current === defaultSettings ? settingsFromPreferences(r.preferences) : current); }).catch(() => {}); return () => { alive = false; }; }, []);
  return <section className="wb-card lab-result"><h2>{job.label}</h2><p><strong>{job.status}</strong> · {job.progress}</p>
    ...
    {result?.complete === false && result?.summary && <Notice error>This run has incomplete or failed cases. Review individual errors; partial averages do not establish an improvement.</Notice>}
    {job.quick && <div className="score-card"><small>QUICK EVALUATION · CURRENT SAVED INSTRUCTIONS</small><strong>{result?.complete ? percent(result.summary?.baseline.pass_rate.mean) : terminal(job.status) ? 'No valid score' : 'Evaluating…'}</strong><p>{result?.complete ? `${job.suite?.length || 0} AI-generated text tests. Review their assumptions and grading evidence before relying on the score.` : 'A score appears only after every sample completes successfully.'}</p></div>}
    {result?.summary?.baseline && <><details open={!job.quick || undefined}><summary>Comparison & run metrics</summary><div className="lab-table-wrap"><table><thead><tr><th>Metric</th><th>{job.quick ? 'With instructions' : 'Original'}</th><th>{job.quick ? 'Without instructions' : 'Candidate'}</th></tr></thead><tbody>
      ...
      </details><p>{job.quick ? 'The comparison measures the same prompts without these instructions. A higher score with instructions suggests value on this generated suite.' : result.summary.delta == null ? 'No comparable score yet.' : `Change: ${(result.summary.delta * 100).toFixed(1)} percentage points.`} Scores apply to this suite and engine, not every use of the skill.</p>
      {!job.quick && !!result.holdout?.baseline.completed && <p>Held-out checks: {percent(result.holdout.baseline.pass_rate.mean)} → {percent(result.holdout.candidate.pass_rate.mean)}.</p>}
    </>}
    {(result?.provenance || result?.provider) && <p>{result.provenance || 'AI evaluation'} · {result.provider || job.settings?.provider}{result.model ? ` · ${result.model}` : ''}{result.engine ? ` · ${result.engine}` : ''}</p>}
    {result && !Number.isFinite(result.cost) && job.settings && <p>Cost not reported, or no model call was made.</p>}
    {result?.cost === null && result.knownCost > 0 && <p>Known portion of cost: ${result.knownCost.toFixed(4)}. Total cost is unavailable.</p>}
    {Number.isFinite(result?.cost) && <p>Reported run cost: ${result.cost.toFixed(4)}{result.engine ? ` · ${result.engine}` : ''}</p>}
    {result?.original !== undefined && <details><summary>Original extracted Markdown</summary><pre>{result.original}</pre></details>}
    ...
    {result?.content && <><pre aria-label="Saved generated draft">{result.content}</pre><Button onClick={() => act(() => copyText(result.content))}>Copy generated draft</Button></>}
    {result?.suite && <><pre>{JSON.stringify(result.suite, null, 2)}</pre><Button onClick={() => onCandidate?.({ ...job, suite: result.suite.evals }, job.source?.content)}>Use generated tests</Button></>}
    {result?.analysis && <pre>{result.analysis}</pre>}
    {!job.quick && job.source?.kind === 'skills' && job.bundleDigest && terminal(job.status) && <Button onClick={() => act(async () => started((await lab('package', { id: job.id })).job.id))}>Create skill package</Button>}
    {result?.package && <Button onClick={() => act(() => native('saveArtifact', { id: job.id, format: 'package' }))}>Save .skill package</Button>}
    {job.quick && result?.complete && onCandidate && <Button primary onClick={() => onCandidate(job, job.source.content)}>Improve using these tests</Button>}
    ...
    {Array.isArray(result?.findings) && <><p>{result.summary}</p>{result.findings.map((f, i) => <article className="wb-card" key={i}><details><summary>Source · line {f.line}</summary><p className="wb-path">{f.path}:{f.line}</p></details><p>{f.message}</p><p>{f.suggestion}</p></article>)}</>}
    {result?.documents?.map((doc, index) => <article className="wb-card" key={index}><h3>{doc.name}</h3>{doc.error ? <Notice error>{doc.error}</Notice> : <>{doc.warnings.map(w => <Notice key={w}>{w}</Notice>)}<Markdown label={`Markdown preview ${doc.name}`} content={doc.content} /><div className="wb-actions"><Button onClick={() => act(() => native('saveArtifact', { id: job.id, format: 'markdown', index }))}>Save Markdown</Button><Button onClick={() => act(() => copyText(doc.content))}>Copy Markdown</Button>{job.kind === 'convert' && <Button onClick={() => setCleanupIndex(index)}>Review AI cleanup options</Button>}</div></>}</article>)}
    ...
    <details><summary>Feedback & notes</summary><Field label="Your feedback"><textarea aria-label="Run feedback" value={feedback} onChange={e => setFeedback(e.target.value)} /></Field><Button onClick={() => act(() => lab('feedback', { id: job.id, content: feedback }))}>Save feedback</Button></details>
    {cleanupIndex !== null && <Dialog title="Clean up extracted Markdown" wide close={() => setCleanupIndex(null)}><p>This sends the selected extracted text to the chosen engine. Original extraction stays in its conversion report; the result is a separate draft.</p><AIControls value={cleanupSettings} change={setCleanupSettings} /><Button onClick={() => act(async () => { const next = await lab('ai.cleanup', { id: job.id, index: cleanupIndex, settings: cleanupSettings }); setCleanupIndex(null); started(next.job.id); })}>Create cleaned draft</Button></Dialog>}
    {change && <Changes source={job.source} content={candidate} run={run} close={() => setChange(false)} />}
  </section>;
}

function TestBuilder({ suite, mode, change, act, promptTemplate }) {
  ...
  return <details className="lab-builder"><summary>Add or import tests</summary>
    ...
    {promptTemplate && <Field label="Prompt variable values (JSON object)"><textarea aria-label="Test variables" value={variables} onChange={e => setVariables(e.target.value)} placeholder={'{"document":"Example input"}'} /></Field>}
...
function SkillForm({ resources, draft, change, act, started, job }) {
  const [advanced, setAdvanced] = useState(draft.advanced === true);
  ...
  const choose = async id => { setLoading(true); try { const r = resources.find(r => r.id === id); if (!r) { set('source', null); return; } const file = await request(r.kind === 'prompts' ? 'prompts.read' : 'resource.read', { id: r.promptId || id, parked: false }); change({ ...draft, source: { ...r, ...file }, candidate: file.content }); } finally { setLoading(false); } };
  ...
  return <section className="wb-card"><div className="segmented" aria-label="Evaluation complexity"><button disabled={loading} aria-pressed={!advanced} onClick={() => { setAdvanced(false); set('advanced', false); }}>Quick evaluation</button><button disabled={loading} aria-pressed={advanced} onClick={() => { setAdvanced(true); set('advanced', true); }}>Advanced</button></div><h2>{advanced ? 'Compare, review and improve instructions' : 'How well does this instruction work?'}</h2>
    {advanced && <Notice>Evaluations compare saved copies using your selected engine. Codex and Cursor run text-only tests. Claude additionally supports restricted file tests and native skill-trigger tests. Shell commands and MCP servers are unavailable. For skills that depend on those tools, inspect capability errors instead of treating the run as a quality score.</Notice>}
    <Field label="Skill, memory, agent, command or prompt"><select aria-label="Evaluation resource" value={source?.id || ''} disabled={loading} onChange={e => act(() => choose(e.target.value))}><option value="">Select an instruction file</option>{resources.filter(r => ['skills', 'memory', 'agents', 'commands', 'prompts'].includes(r.kind) && !r.parked && !r.error).map(r => <option key={r.id} value={r.id}>{r.name} · {r.provider} · {r.scope}</option>)}</select></Field>
    {loading && <p role="status">Loading selected instructions…</p>}
    {!advanced && <><p>Pick a resource. AIOS designs four text tests, runs them with your saved engine, and shows the score with evidence. It also tests the same prompts without the instructions for comparison.</p><div className="quick-steps"><span>1 · Generate tests</span>→<span>2 · Run in background</span>→<span>3 · Review the score</span></div>
      <p className="wb-muted">Uses your model service and tokens. Generated tests are a starting point, not a certification. Files, MCP and shell access are excluded; use Advanced for supported file tests.</p><Button primary disabled={!source || loading} onClick={() => act(async () => started((await lab('evaluate.quick', { id: source.id, settings: config, guidance: draft.feedback || '' })).job.id))}>Run quick evaluation</Button><details><summary>Choose engine & testing goal</summary><fieldset disabled={loading} className="lab-instruction-fields"><Settings value={config} change={settings => set('settings', settings)} /><Field label="What matters most?"><textarea value={draft.feedback || ''} onChange={e => set('feedback', e.target.value)} /></Field></fieldset></details></>}
    {source && advanced && <fieldset disabled={loading} className="lab-instruction-fields"><details><summary>Source details</summary><p className="wb-path">{source.path}</p></details>{job?.source?.id === source.id && job.result?.summary?.baseline ? <Notice>Latest recorded comparison: Original {percent(job.result.summary.baseline.pass_rate.mean)} → Candidate {percent(job.result.summary.candidate.pass_rate.mean)}. Rerun after changing the draft, tests or model.</Notice> : <p>Current score: Not evaluated for this draft and suite. Run results are recorded below.</p>}
      ...
      {source.kind === 'prompts' && <p>For templates such as {'{{document}}'}, supply a variables object in every test, for example {'{"variables":{"document":"Example input"}}'}. Both versions use the same values. Missing values stop the run before any model call.</p>}
      <TestBuilder suite={suite} mode={mode} promptTemplate={source.kind === 'prompts'} change={value => set('suite', value)} act={act} />
      ...
      <div className="wb-actions"><Button primary onClick={() => start(mode)}>Run comparison</Button><Button onClick={() => start('improve')}>Improve and retest</Button><Button onClick={() => start('proposal')}>Draft from feedback</Button><Button onClick={() => act(async () => started((await lab('ai.suite', { id: source.id, mode, settings: config, goal: draft.feedback || '' })).job.id))}>Generate test cases</Button></div>
...
function MemoryForm({ resources, projects, draft, change, act, started, run, evaluate, busy }) {
  ...
  return <><section className="wb-card"><h2>Review memory and rules</h2><p>Select the files to review together. Scope is discovered from disk; providers decide what is actually loaded into each session.</p>
    ...
    <div className="lab-memory-files">{files.map(f => <label className="wb-check" key={f.id}><input type="checkbox" checked={ids.includes(f.id)} onChange={e => set('ids', e.target.checked ? [...ids, f.id] : ids.filter(id => id !== f.id))} /><span>{f.name} · {f.provider} · {f.scope}</span></label>)}</div>
    ...
    <details><summary>Advanced memory options</summary><div className="wb-form-grid"><Field label="Length guidance (lines)"><input aria-label="Memory line guidance" type="number" min="20" max="5000" value={draft.maxLines ?? 500} onChange={e => set('maxLines', Number(e.target.value))} /></Field>
      <Field label="Project for scope suggestions"><select aria-label="Memory review project" value={draft.project || ''} onChange={e => set('project', e.target.value)}><option value="">No project selected</option>{projects.map(p => <option key={p}>{p}</option>)}</select></Field></div></details>
    ...
    <Button disabled={!ids.length || busy} onClick={() => act(async () => started((await lab('memory.ai', { ids, settings: config })).job.id))}>Enhance review with AI</Button>
    ...
    {review && <section className="wb-card"><h2>Local review results</h2><p>{review.documents.length} files · {review.metrics.lines} lines · approximately {review.metrics.estimatedTokens} text tokens · {review.findings.length} findings</p><p>These are document measurements, not model performance scores. Suggestions preserve the original until you review and apply a change.</p>
      ...
      {review.findings.map(f => <article className="wb-card" key={f.id}><span className="wb-badge">{f.code}</span><details><summary>Source · line {f.line}</summary><p className="wb-path">{f.path}:{f.line}</p></details><p>{f.message}</p>
      ...
      {review.documents.map(source => <details key={source.id}><summary>{source.path} · {source.metrics.lines} lines</summary><Markdown content={source.content} />{source.scope === 'user' && !source.readonly && source.sections.map(s => <div className="wb-row" key={s.start}><span>{s.title} · lines {s.line}–{s.endLine}</span><Button onClick={() => setMove({ source, section: s })}>Move section</Button></div>)}</details>)}
    </section>}
...
export function Labs({ page, data, resources, run, jobsState, drafts, setDrafts, navigate }) {
  ...
  const draft = { settings: settingsFromPreferences(data.preferences?.ai), ...drafts[page] }, change = value => setDrafts(prev => ({ ...prev, [page]: value }));
  ...
  useEffect(() => { if (job?.id) document.querySelector('.lab-result')?.scrollIntoView({ block: 'start' }); }, [job?.id, job?.status]);
  ...
  const kinds = page === 'convert' ? ['convert', 'install', 'cleanup'] : page === 'memory-review' ? ['memory-review', 'proposal'] : ['evaluate', 'trigger', 'improve', 'proposal', 'analysis', 'package', 'author', 'review', 'suite'];
  return <div className="lab-workspace" aria-busy={busy}>
    ...
    {job && <RunResult key={job.id} job={job} act={act} run={run} started={started} onCandidate={(j, candidate) => { setDrafts(prev => ({ ...prev, 'skill-lab': { source: j.source, candidate, suite: JSON.stringify({ evals: j.suite || [] }, null, 2), mode: j.mode || 'evaluate', advanced: true, settings: j.settings, feedback: j.feedback || '' } })); navigate('skill-lab'); }} />}
    <details className="wb-card"><summary><strong>Run history</strong></summary>{!jobsState.jobs.some(j => kinds.includes(j.kind)) && <p>No runs yet.</p>}{jobsState.jobs.filter(j => kinds.includes(j.kind)).map(j => <div className="wb-row" key={j.id}><Button onClick={() => setSelected(j.id)}>{j.label} · {j.status}</Button><span>{new Date(j.createdAt).toLocaleString()}</span>{terminal(j.status) && <Button onClick={() => { if (window.confirm('Delete this run and its saved input/output copies? Installed resources will remain unchanged.')) act(async () => { await lab('delete', { id: j.id }); if (selected === j.id) setSelected(null); }); }}>Delete run</Button>}</div>)}</details>
  </div>;
}
