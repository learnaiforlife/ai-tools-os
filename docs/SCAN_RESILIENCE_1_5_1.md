...
| Slow resource loading can overwrite an in-progress Skill Lab edit | Candidate, suite and run controls now stay locked while a newly selected resource loads. A delayed native-read UAT regression verifies the lock. |
