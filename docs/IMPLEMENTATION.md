...
The declared target is macOS 13 or later on Apple Silicon and Intel. The beta path produces complete ad-hoc signatures, verifies both installer architectures and runs packaged UI acceptance checks without an Apple certificate. Its exact uploaded bytes must match public checksums before the beta manifest is published. macOS may require app-opening approval and, separately, access to selected scan folders. The stable path requires a clean reviewed commit, Developer ID signing, notarization/stapling and Gatekeeper acceptance. Neither path establishes untested minimum-OS or historical-upgrade coverage. The site renders only its verified release manifest and never copies unreviewed public assets.
